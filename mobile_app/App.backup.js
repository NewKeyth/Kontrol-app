...
const TEMAS = {
  verde: {
    nombre: 'Verde', glow: '#6EE891', accent: '#89B990', accentDark: '#4A634E',
    btnBase: '#2A3B2E', btnBaseSoft: '#1E2B22', btnActive: '#4A8B55', btnActiveSoft: '#3D7348',
    surface: '#1A261D', surfaceDeep: '#141E17', bg: '#0F1711', border: '#243628', text: '#E0EFE1',
  },
  azul: {
...
const MotorTactilMaestro = ({ onBotonPresionar, onBotonSoltar, onJoyMover, onJoyActivar, onJoyDesactivar, salirDelJuego, conectado, intVibracion, tema }) => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: tema.bg }} onLayout={calibrar}>
      ...
      <View ref={refs.LT} style={[{ position: 'absolute', backgroundColor: tema.btnBase, width: 80, height: 48, borderRadius: 14, justifyContent: 'center', alignItems: 'center', elevation: 4, overflow: 'hidden', top: 10, left: 20 }, ba['LT'] && btnAct]} pointerEvents="none">
      ...
      <View ref={refs.RB} style={[{ position: 'absolute', backgroundColor: tema.btnBaseSoft, width: 65, height: 40, borderRadius: 12, justifyContent: 'center', alignItems: 'center', elevation: 3, overflow: 'hidden', top: 10, right: 115 }, ba['RB'] && btnActS]} pointerEvents="none">
        <GlowInterno activo={!!ba['RB']} borderRadius={12} color={tema.glow} /><Text style={[{ color: tema.accent, fontWeight: 'bold', fontSize: 15 }, ba['RB'] && { color: '#FFF', textShadowColor: tema.glow, textShadowRadius: 15, textShadowOffset: { width: 0, height: 0 } }]}>RB</Text>
      </View>
      ...
      <View ref={refs.JOY_DER} style={{ position: 'absolute', bottom: 25, right: 200 }} pointerEvents="none">
        <View style={{ width: 110, height: 110, borderRadius: 55, backgroundColor: tema.surfaceDeep, borderWidth: 2, borderColor: tema.border, justifyContent: 'center', alignItems: 'center', elevation: 2 }}>
...
// ═══════════════════════════════════════════════
// 📱 COMPONENTE PRINCIPAL (SIN SAFE AREA AÚN)
// ═══════════════════════════════════════════════
function MainApp() {
        ...
        <View style={{ flex: 1, padding: 20 }}>
          ...
          {pestanaActual === 'Inicio' && (
            <ScrollView style={{ flex: 1, marginTop: 10 }} showsVerticalScrollIndicator={false}>
              ...
              <Text style={{ fontSize: 18, color: tema.accent, fontWeight: 'bold', marginBottom: 15 }}>Diseños</Text>
              
              {/* Game Mode Cards - Using original components */}
              <TouchableOpacity
                onPress={() => entrarAlMando('estandar')}
                activeOpacity={0.8}
                style={{ width: '100%', borderRadius: 20, marginBottom: 20, overflow: 'hidden' }}
              >
                <View style={{ backgroundColor: tema.surface, borderRadius: 20, padding: 15 }}>
                  <View style={{ backgroundColor: tema.bg, borderRadius: 15, marginBottom: 10, overflow: 'hidden' }}>
                    <MiniPreviewClasico tema={tema} />
                  </View>
                  <Text style={{ fontSize: 16, fontWeight: 'bold', color: tema.text }}>Mando Clásico</Text>
                </View>
              </TouchableOpacity>
              ...
              >
                <View style={{ backgroundColor: tema.surface, borderRadius: 20, padding: 15 }}>
                  ...
                  <Text style={{ fontSize: 16, fontWeight: 'bold', color: tema.text }}>Carreras Pro</Text>
                </View>
              </TouchableOpacity>
            </ScrollView>
          )}
          ...
          {pestanaActual === 'Conectar' && (
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
              ...
              <View style={{ marginTop: 30, alignItems: 'center', width: '100%' }}>
                      ...
                      {[1, 2, 3, 4].map((num) => {
                        ...
                        return (
                          <TouchableOpacity
                            ...
                            disabled={estadoConexion === 'conectando' || estadoConexion === 'conectado'}
                            style={{
                              width: 56, height: 56, borderRadius: 28,
                              backgroundColor: isSelected ? tema.glow : tema.surface,
                              borderWidth: 2,
                              borderColor: isSelected ? tema.glow : (isTaken ? '#FF4444' : tema.accentDark),
                              justifyContent: 'center', alignItems: 'center',
                              opacity: (estadoConexion === 'conectando' || estadoConexion === 'conectado') ? 0.5 : 1,
                            }}
                          >
                            <Text style={{ 
                              color: isSelected ? tema.bg : tema.text, 
                              fontSize: 24, fontWeight: 'bold' 
                            }}>{num}</Text>
                            {isTaken && (
                              <View style={{ position: 'absolute', top: -4, right: -4, width: 14, height: 14, borderRadius: 7, backgroundColor: '#FF4444', justifyContent: 'center', alignItems: 'center' }}>
                ...
                {connectedPlayers.includes(playerId) && (
                  <Text style={{ color: '#FF4444', fontSize: 12, marginTop: 8 }}>Este jugador ya está conectado</Text>
                )}
              </View>
              ...
              <View style={{ marginTop: 30, alignItems: 'center', width: '100%' }}>
                {!mostrarCampoIp && ipGuardada ? (
                  ...
                  <View style={{ alignItems: 'center', width: '100%' }}>
                    <TextInput style={{ width: '85%', backgroundColor: tema.surface, color: tema.text, fontSize: 18, padding: 15, borderRadius: 15, borderWidth: 1, borderColor: tema.accentDark, textAlign: 'center' }}
            ...
            <ScrollView style={{ flex: 1, marginTop: 10 }} showsVerticalScrollIndicator={false}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
                {Object.keys(TEMAS).map(k => (
                  <TouchableOpacity
                    key={k}
                    onPress={() => guardarTema(k)}
                    variant={temaActual === k ? 'primary' : 'secondary'}
                    style={{
                      width: 70, height: 70, borderRadius: 20,
                    }}
                  >
                    <View style={{ justifyContent: 'center', alignItems: 'center', flex: 1 }}>
                      <View style={{ width: 30, height: 30, borderRadius: 15, backgroundColor: TEMAS[k].glow }} />
                      <Text style={{ color: temaActual === k ? tema.bg : TEMAS[k].accent, fontSize: 10, marginTop: 4, fontWeight: 'bold' }}>{TEMAS[k].nombre}</Text>
                    </View>
                  </TouchableOpacity>
                ))}
              </View>
              ...
              {/* ABOUT (KONTROL LOGO) - Using AnimatedCard (7.2) */}
              <View style={{ padding: 20, alignItems: 'center', marginBottom: 15 }}>
                <Image 
                  ...
                  resizeMode="contain" 
                />
                <Text style={{ color: tema.text, fontSize: 22, fontWeight: 'bold', marginTop: 10 }}>KONTROL</Text>
